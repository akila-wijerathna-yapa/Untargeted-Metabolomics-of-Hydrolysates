...
# Load data - Adjust paths as the .Rmd file is in D:\Untargeted-Metabolomics-of-Hydrolysates\Syntax
...
cat("Structure of df_combined (initial):\n")
str(df_combined)
```
...
cat("\nStructure of df_avg after joining:\n")
str(df_avg)
```
...
# Variance explained (from previous PCA calculation)
# pc1_var and pc2_var are already calculated
...
p.heat <- pheatmap(
  m.int,
  scale = "row", # Scales rows to show relative changes for each metabolite
  show_rownames = FALSE,
  cluster_rows = TRUE,
  cluster_cols = TRUE,
...
# Printing the pheatmap object will render it in the HTML
print(p.heat)
...
cat("Top 50 Metabolites per Group:\n")
...
# Store metabolite names separately (optional, not strictly needed for the plot but good for context)
metabolite_names_for_upset <- upset_data$Metabolite.name

# Drop the name column to retain just the presence/absence matrix for intersection calculation
upset_matrix <- upset_data %>%
  select(-Metabolite.name)

# Generate and print the Upset plot
ComplexUpset::upset(
  data = upset_data,
